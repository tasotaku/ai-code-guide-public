...
// 1. 未設定は undefined
{
    ...
    assert.strictEqual(s.get("file://a.py", "id1"), undefined, "未設定は undefined");
    ok("未設定は undefined");
}
...
{
    const s = new AnnotationStatusStore(dir);
    s.set("file://a.py", "id1", "hidden");
    assert.strictEqual(s.get("file://a.py", "id1"), "hidden", "保存した状態が返る");
...
// 3. ディスクに永続化され、別インスタンスでも読める
{
    ...
    assert.strictEqual(s2.get("file://a.py", "id1"), "hidden", "再構築後も状態が残る");
    ok("永続化: 別インスタンスでも読める");
}
...
// 4. null で非表示を解除する（削除）
{
    ...
    assert.strictEqual(s.get("file://a.py", "id1"), undefined, "null で非表示を解除する");
    const s2 = new AnnotationStatusStore(dir);
    assert.strictEqual(s2.get("file://a.py", "id1"), undefined, "削除も永続化される");
    ok("null で非表示を解除し、削除も永続化");
}

// 5. uri で名前空間が分かれる（別ファイルの同一 id は混ざらない）
{
    const s = new AnnotationStatusStore(dir);
    s.set("file://a.py", "same", "hidden");
    s.set("file://b.py", "same", "hidden");
    assert.strictEqual(s.get("file://a.py", "same"), "hidden", "a.py は hidden");
    assert.strictEqual(s.get("file://b.py", "same"), "hidden", "b.py は hidden");
    ok("uri で名前空間が分かれる");
}
...
// 6. 上限(MAX_ENTRIES=5000)超過で最古が捨てられ、最近のものは残る(FIFO eviction)
{
    ...
    for (let i = 0; i < 5001; i++) s.set("file://cap.py", `id${i}`, "hidden");
    assert.strictEqual(s.get("file://cap.py", "id0"), undefined, "最古(id0)は eviction で消える");
    assert.strictEqual(s.get("file://cap.py", "id5000"), "hidden", "最新(id5000)は残る");
    // 既存キーの再setは「最近使った」扱いで生き残る(delete→set し直すため)
    const s2 = new AnnotationStatusStore(dir6);
    s2.set("file://cap.py", "id1", "hidden"); // 触れて末尾へ寄せる
    s2.set("file://cap.py", "idNew", "hidden"); // 1件追加 → 最古(id2)が押し出される
    assert.strictEqual(s2.get("file://cap.py", "id1"), "hidden", "再set した id1 は生き残る");
